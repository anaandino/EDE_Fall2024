...
class(NTL.phys.data$year4)  #the variable is an integer
NTL.phys.data.yearfilter <- filter(NTL.phys.data, year4 %in% 1999)
Ex1 <- filter(NTL.phys.data, year(sampledate) == 1999)
view(NTL.phys.data.yearfilter)
...
> Answer: We do not have contorl over row numbers 
...
Ex3 <- NTL.phys.data %>% 
  filter(lakename == 'Tuesday Lake') %>% 
  filter(year4 %in% c(1990:1999))%>% 
  mutate(month = month(sampledate) == 7)
