...
```{r, results = "hide"}
vignette("dplyr")
##Gives you information about the function. Go to single table verbs. Three types: Rows (filter, slice, arrange), Columns (select, rename, mutate,relocate) and Groups Rows(summarise)
```
...
# 1st option - matrix filtering {Here filtering rows that are 0 and no rule for colums so its added before ","}
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# 2nd option -dplyr filtering
...
##%in% instead ofwriting all the names many times##
...
*TIP*: if we do all in the same file it could become too big and then you can get lost when finding code. so divide wrangling from analysis process. 
