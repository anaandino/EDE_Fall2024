...
#combined vectors but did not assign it. If I had asgined then it would appear on the Environment 
c(vector1, vector4)

#My ouput will be another vector
vector1 * vector4
...
#Gives the opposire of vector 3
!vector3
...
matrix1[4, 1] #4th row and 1st column

matrix1matrix1[4,  ] #just the 4th row

matrix1[ , 3] #will give 3rd column of the matrix

matrix1[c(12, 14)] #specific position

matrix1[c(12:14)] #C gives the 14th or 12th position of the matrix but square brackets are more useful

matrix1[2:4, 1:3] #from matrix 1 I might want row 2,3,4 and columns 1,2,3. Basically making a subset. 
...
cnames <- c("C1", "C2", "C3") #must use quotes because they are names for rows and columns
...
#R gives all elements in list
list1

#if only inetrested in age. dollar sign gives you all the object in the list and you choose one
list1$age

#I will all the names in the list
names(list1)

#get first two objects on my list one
list1[1:2]

#change the name from maria to janelle
list1$name = "Janelle"

#gives just the value. just one bracket gives you another thinkg than two brackets. change the value of the list 
list1[[4]] = 44
```
...
#Combine the vectors into a dataframe. dataframe need to have same number of rows 
