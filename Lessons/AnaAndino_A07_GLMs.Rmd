...
#upload raw data
LakeChemistryPhysics <- read.csv(here("Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"), stringsAsFactors = TRUE)
head(LakeChemistryPhysics$sampledate)
...
#2nd option:
LakeChemistryPhysics$sampledate <- mdy(LakeChemistryPhysics$sampledate)
...
plot1 <- ggplot(Lake.subset,
  aes(x= depth, y= temperature_C))+
    geom_point(alpha = 0.5, size = 3, color = "white")+
    geom_smooth(method = lm, se = FALSE, color = "orange", size = 0.8)+
  ylim(0 , 35)+
   labs(
...
reg.depth <- lm(temperature_C ~ depth, data = Lake.subset)
...
#P-values very small thus reject the null hypothesis and conclude that at least one lake's average temperature differs from the others
...
cat("Lakes with the same mean temperature as Peter Lake:\n")
...
>Answer: Pual Lake and Ward Lake have a mean temperatute that is statistically the same as Peter Lake and there is no lake that has a statistically distinct mean temperature from all others,
