...
plot.density <- ggplot (EPAair,
                      aes(x = Ozone, fill = as.factor(Year))) + # since the graph is density then we do not need to specify y
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("darkgreen", "darkblue"))+
  labs (x= "Ozone AQI values",
        y ="Density",
        fill = "")
print(plot.density)

boxplot.plot <- ggplot(EPAair, aes(x= as.factor(Year),
                              y = Ozone)) +
  geom_boxplot()+
...
#The p-value is 2.2e-16 which tells it a good model and r-squared is 0.13 - there is a relationship but x is explaining only 13% of y so we need more predictors to explain the relationship.
...
RegPlot <- ggplot(EPAair, aes(x=Ozone, y= PM2.5))+
  geom_point()+
  geom_smooth(method = "lm", col = "blue")+
  geom_text(x=100, y=75, label = expression ("PM2.5=0.38*Ozome +5.6"))
print(RegPlot)
